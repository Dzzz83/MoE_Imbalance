...
def test_epoch_metrics_sink_flushes_jsonl_without_changing_training():
    """Each completed epoch is durable while the CPU train step stays finite."""
    import json

    from expert_method.attempts import AttemptWorkspace

    with tempfile.TemporaryDirectory(prefix="dsh_metrics_") as tmp:
        ...
        trainer = _TinyTrainer(
            device="cpu",
            epochs=2,
            checkpoint_dir=str(attempt.path / "checkpoints"),
            metrics_sink=lambda event: _write_and_check_flushed(attempt, event),
        )
        initial = trainer.model.weight.detach().clone()
        history = trainer.train(_tiny_loader(seed=7))
        events = [json.loads(line) for line in attempt.metrics_path.read_text().splitlines()]

        torch.manual_seed(23)
        baseline = _TinyTrainer(
            device="cpu", epochs=2, checkpoint_dir=str(attempt.path / "baseline")
        )
        baseline.train(_tiny_loader(seed=7))

        assert [event["epoch"] for event in events] == [1, 2]
        assert [event["epoch"] for event in history] == [1, 2]
        assert torch.isfinite(torch.tensor([event["train_loss"] for event in events])).all()
        assert not torch.equal(initial, trainer.model.weight.detach())
        assert torch.equal(baseline.model.weight.detach(), trainer.model.weight.detach())
        assert trainer.model.weight.grad is not None
        assert torch.isfinite(trainer.model.weight.grad).all()


def _write_and_check_flushed(attempt, event):
    attempt.metrics_sink(event)
    events = [line for line in attempt.metrics_path.read_text().splitlines()]
    assert len(events) == event["epoch"]
...
def test_synthetic_dry_run_all_four_experts():
    ...
    for name, fname in configs.items():
        cfg_path = os.path.join(_proj_root, 'configs', 'experts', fname)
...
def test_seed_controls_model_initialisation():
    ...
    cfg = TrainingConfig.from_file(
        os.path.join(_proj_root, 'configs', 'experts', 'ce.yaml')).replace(device='cpu')
...
def test_same_seed_gives_an_identical_run():
    ...
    for tag in ('a', 'b'):
        cfg = TrainingConfig.from_file(
            os.path.join(_proj_root, 'configs', 'experts', 'ce.yaml')).replace(
...
def test_different_seeds_give_different_inits():
    ...
    counts = np.array([500] + [5] * 99)
    path = os.path.join(_proj_root, 'configs', 'experts', 'ce.yaml')
