...
- **Project Goal:** Achieve ≥1% Balanced Accuracy gain over uniform averaging via dynamic routing across three diverse experts (LAL, PaCo, Mixup).
- **Current Phase:** Routing evaluation on corrected data pipeline; boosting and novel routing ideas under consideration.
...
- **[LAL]**: Logit-Adjusted Loss — adds `τ·log(π_y)` to logits where π_y is the class prior (Menon ICLR 2021).
- **[PaCo]**: Parametric Contrastive Learning — supervised contrastive loss with learnable class centers (Cui ICCV 2021).
- **[Mixup]**: Mixup-augmented Cross-Entropy — trains on convex combinations of image pairs (Zhang ICLR 2018).
- **[BA]**: Balanced Accuracy — per-class accuracy averaged across all classes (not overall accuracy).
- **[ECE]**: Expected Calibration Error — |accuracy − confidence|, averaged over bins.
- **[IR]**: Imbalance Ratio — max class count / min class count (100:1 in this project).
- **[Head/Med/Tail]**: Class groups by frequency: Head ≥100, Med 20-100, Tail <20 training samples.
...
All project-specific information (architecture, code structure, current status, results, and forward plans) is in the `docs/` directory. After loading this system prompt, read `docs/project_overview.md` next to understand the project context.
